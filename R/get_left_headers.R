...
#' @export
get_left_headers <- function(dat, first_right_header_col, left_headers) {
  
  # to avoid errors in assigning names to columns, make sure cols are ordered
  ordered_cols <- arrange(dat, col)
  
  first_data_row <- get_first_data_row(ordered_cols)
  ...
  # Assume that there is only ever one row of left block headers, and that they
  # are directly above the data.
  if (any(!is.na(non_blank_names$character))) {
    ...
    existing_names_last_row <- non_blank_names %>% 
      filter(row == header_row | is.na(row))
    
    names_excluding_blanks <- existing_names_last_row %>% 
      filter(is.na(character) == FALSE)
    
    all_names_exist <- nrow(names_excluding_blanks) == nrow(col_locs_to_name)
    some_names_exist <- any(!is.na(existing_names_last_row$character))
  ...
  if (all_names_exist) {
    cols_to_name <- existing_names_last_row$character
    
  } else if (some_names_exist) {
    cols_to_name <- existing_names_last_row %>% 
