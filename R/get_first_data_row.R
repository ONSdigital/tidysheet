...
#' 
#' @export
get_first_data_row <- function(dat) { 
  ...
  numeric_rows <- get_vector_locs_of_type(dat, "numeric", tolerance = 0.4, "row")
  character_rows <- get_vector_locs_of_type(dat, "character", tolerance = 0.7, "row")
  
  # See documentation for amend_numeric_rows to understand why we care about 
  # blank rows
  blank_rows <- get_vector_locs_of_type(dat, "blank", tolerance = 0.9, "row")
