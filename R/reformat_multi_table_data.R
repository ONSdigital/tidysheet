...
#' 
#'
#' @param source_data tidyxl::xlsx_cells table. 
...
#'
#' @export
reformat_multi_table_data <- function(source_data, 
                                      table_split_points, 
  ...
  partitioned_data <- split_table(source_data, split_pattern)
  ...
  for (i in 1:tables_count) {
    ...
    # However, if a table does contain numbers it needs to be pre-processed.
    table_contains_numbers <- any(!is.na(for_beheading$numeric))
    if (table_contains_numbers) {  
    ...
    } else if (table_contains_numbers == FALSE){
