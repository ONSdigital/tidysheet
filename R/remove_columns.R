...
#' rectify(output)
#' @export
remove_columns <- function(dat, column_regex) {

  if (all(is.na(column_regex))) {
    return(dat)
  }

  columns_to_remove <- identify_columns_to_remove(dat, column_regex)
  ...
  if (length(columns_to_remove) > 0) {
    ...
    warning(paste0("The following column(s) from the raw data have been removed in the pre-processed data, ",
                   "as specified in the cols_to_remove_regex argument in the data dictionary: ",
                   column_letters_removed))
