#' @title reformat_no_row_headers_data
#' @description Reformat no_row_headers data
#' @author Emma Wood \email{emma.wood@ons.gov.uk}
#' @author Mark London \email{mark.london@ext.ons.gov.uk}
#' @details 
#' Reformat no_row_headers data
#' 
#' Used for cases where Excel data have only one header row, with one column
#' of strings in the first non-empty column. 'Total group' items are included in 
#' the first non-blank column - these are the groups under which the nested_row_1
#' is nested.
#' 
#' Reformat these data so that there is only one number per row, and 
#' the group of 'total group' is associated with all the relevant entries.
#' 
#' 
#' @param main_table A dataframe containing the main data table to be processed.
#' @param group_row string. New column name for the groups the nested_row_1 
#' entries belong to.
#' @param nested_row_1 string. New column name for the entries in the first 
#' non-empty column.
#' @param group_col string. New column name for the header row 
#' (all columns except the first non-empty column.
#' 
#' @return A data frame containing the unpivotted data.
#' 
#' @examples
#' sample_data <- data.frame(
#'   address = c('B2', 'C2', 'A3', 'B3', 'C3', 'A4', 'B4', 'C4', 'A5', 'B5', 'C5'),
#'   row = c(2, 2, rep(3:5, each = 3)),
#'   col = c(2, 3, rep(1:3, 3)),
#'   is_blank = rep(FALSE, 11),
#'   data_type = c(rep('character', 3), 
#'               rep(c(rep('numeric', 2), 'character'), 2), rep('numeric', 2)),
#'   numeric = c(rep(NA, 3), 1, 2, NA, 3, 4, NA, 5, 6),
#'   character = c('measure 1', 'measure 2', 'primary', rep(NA, 2),
#'                 'secondary', rep(NA, 2), 'Total education', rep(NA, 2))
#' )
#' 
#' # look at the data as it would be in Excel
#' rectify(sample_data)
#' 
#' reformat_no_row_headers_data(sample_data, 'service', 'subservice', 'transaction')
#' 
#' @export
reformat_no_row_headers_data <- function(main_table, group_row, nested_row_1, group_col) {
  
  beheaded <- simple_table_behead(main_table, nested_row_1, group_col)
  
  beheaded_plus_group <- beheaded %>% 
    # clean up nested_row_1 so that regex patterns in str_detect and str_remove are less complex
    mutate(!!sym(nested_row_1) := 
             str_squish(str_to_lower(!!sym(nested_row_1)))
    ) %>% 
    # if 'total' is in the entry for nested_row_1, this is the grouping variable,
    # so we can get it by removing 'total' from the string
    mutate(!!sym(group_row) := ifelse(
      str_detect(!!sym(nested_row_1), "^total"),
      str_squish(str_remove(!!sym(nested_row_1), "^total")),
      NA)
    ) %>% 
    # so far the group is only given for the total of that group, so we need to 
    # fill in the NAs. This assumes that the total is given after the items that 
    # make up the total
    fill(!!sym(group_row), .direction = "up") %>% 
    # some items may not have any sub-items, for these we can assume that the 
    # nested item and the group are the same
    mutate(!!sym(group_row) := ifelse(is.na(!!sym(group_row)),
                                      !!sym(nested_row_1), 
                                      !!sym(group_row)))
  
  return(beheaded_plus_group)
  
}