...
#'
#' @export
get_header_identifier <- function(header_identifier, file_part) {

  file_part_numeric <- as.numeric(file_part)

  if (between(file_part_numeric, 1, length(header_identifier))){
    header_identifier <- header_identifier[file_part_numeric]
  }
