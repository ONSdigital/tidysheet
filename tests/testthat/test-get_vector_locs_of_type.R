...
test_that("get_vector_locs_of_type returns expected result", {
  ...
  result_na_included_in_tolerance_calc <- get_vector_locs_of_type(
    dat, "numeric", 0.9
  )
...
test_that("get_vector_locs_of_type raises an error if tolerance is not between 0 and 1", {
  expect_error(
    get_vector_locs_of_type(dat, "character", 1.1),
    "tolerance must be between 0 and 1"
    )
  expect_error(
    get_vector_locs_of_type(dat, "character", -0.4),
    "tolerance must be between 0 and 1"
  )
})
